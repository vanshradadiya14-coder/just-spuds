 ...
 * IMAGE SOURCES
 * -------------
 * Every photograph exists in two places: on the generator's CDN, and
 * (optionally) as a local file once `npm run assets` has been run.
 *
 * The site reads the CDN by default so the photographs appear with no setup at
 * all. If a local copy exists it is preferred, because it is faster and does
 * not depend on someone else's CDN staying up.
 *
 * `resolve()` returns both, and SmartImage falls back from one to the other.
 */
const CDN = 'https://d8j0ntlcm91z4.cloudfront.net/user_3HQOMRVSN4opWpwTrdyODLFGEQP/'

/** local path (under /assets) → CDN filename */
...
export interface ImageSource {
  /** Tried first. */
  src: string
  /** Used if the first 404s. */
  fallback?: string
}

/** Accepts either a bare key or a full `/assets/...` path. */
export function resolve(pathOrKey: string): ImageSource {
  if (!pathOrKey) return { src: '' }
  if (/^https?:\/\//.test(pathOrKey)) return { src: pathOrKey }

  const key = pathOrKey.replace(/^\/?assets\//, '')
  const remote = IMAGE_MAP[key]

  // Prefer local image assets so they load fast and reliably without external CDN delays.
  return { src: `/assets/${key}`, fallback: remote ? CDN + remote : undefined }
}
